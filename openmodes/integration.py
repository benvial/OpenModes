...
def cartesian_to_barycentric(r, nodes):
    ...
    T = np.array(
        (
            (nodes[0, 0] - nodes[2, 0], nodes[1, 0] - nodes[2, 0]),
...
def triangle_electric_dipole(vertices, xi_eta, weights):
    ...
    r = (
        (vertices[0] - vertices[2]) * xi_eta[:, 0, None]
...
class Contour(object):
    ...
    def points_inside(self, points):
        ...
        inside = np.empty(np.prod(points.shape), dtype=bool)
...
class ExternalModeContour(Contour):
    ...
    def __init__(
        ...
        if corner.real >= 0.0 or corner.imag <= 0.0:
            raise ValueError(
                "Corner frequency must have negative real "
