...
from openmodes.eig import ConvergenceError, eig_linearised, eig_newton, poles_cauchy
...
class Operator(object):
    ...
    def impedance(self, s, parent_o, parent_s, metadata=None):
        ...
        Z = self.impedance_class(
            parent_o, parent_s, self.basis_container, self.sources, self.unknowns
        )
    ...
    def estimate_poles(
        ...
        **kwargs
        ...
        if not cauchy_integral:
            ...
            result = poles_cauchy(
                Z_func, contour, threshold, previous_result=previous_result, **kwargs
            )

        return result

    def refine_poles(self, estimates, part, rel_tol, max_iter, iter_wrap=lambda x: x):
