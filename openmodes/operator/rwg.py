...
def impedance_curl_G(
    ...
    if self_impedance:
        ...
        singular_terms = singular_impedance_rwg(
            basis_o,
        ...
        res = z_mfie_faces_self(
            nodes_o,
            basis_o.mesh.polygons,
            basis_o.mesh.polygon_areas,
            gamma_0,
    ...
    Z = transform_o.dot(
        transform_s.dot(
            Z_faces.reshape(num_faces_o * 3, num_faces_s * 3, order="C").T
        ).T
    )

    Z_dgamma = transform_o.dot(
        transform_s.dot(
            Z_dgamma_faces.reshape(num_faces_o * 3, num_faces_s * 3, order="C").T
        ).T
    )
...
def impedance_G(
    ...
    if self_impedance:
        ...
        if np.any(np.isnan(singular_terms[0])) or np.any(np.isnan(singular_terms[1])):
            raise ValueError("NaN returned in singular impedance terms")

        num_faces_s = num_faces_o
        res = z_efie_faces_self(
            nodes_o,
            basis_o.mesh.polygons,
            gamma_0,
            integration_rule.points,
    ...
    L = transform_L_o.dot(
        transform_L_s.dot(
            A_faces.reshape(num_faces_o * 3, num_faces_s * 3, order="C").T
        ).T
    )
    ...
    dL_ds = transform_L_o.dot(
        transform_L_s.dot(
            A_dgamma_faces.reshape(num_faces_o * 3, num_faces_s * 3, order="C").T
        ).T
    )
