...
def generate_mesh():
    ...
    meshed_name = gmsh.mesh_geometry(
        osp.join(openmodes.geometry_dir, "srr.geo"), input_dir, parameters=parameters
    )
...
def srr_pair_combined_poles(plot=False):
    ...
    sim = openmodes.Simulation(basis_class=LoopStarBasis, operator_class=EfieOperator)
