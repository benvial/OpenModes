...
from helpers import read_1d_complex, read_2d_real, write_1d_complex, write_2d_real
...
def assert_allclose_sign(a, b, rtol):
    ...
    assert np.all(np.abs(a - b) < rtol * abs(a)) or np.all(
        np.abs(a + b) < rtol * abs(a)
    )
...
def test_horseshoe_modes(plot=False, skip_asserts=True, write_reference=False):
    ...
    if write_reference:
        write_1d_complex(
            osp.join(reference_dir, "eigenvector_0.txt"), mode_j["J", :, "modes", 0]
        )
        write_1d_complex(
            osp.join(reference_dir, "eigenvector_1.txt"), mode_j["J", :, "modes", 1]
        )
        write_1d_complex(
            osp.join(reference_dir, "eigenvector_2.txt"), mode_j["J", :, "modes", 2]
        )
    ...
    if not skip_asserts:
        assert_allclose(
            mode_s[0],
            [
                -2.585729e09 + 3.156438e10j,
                -1.887518e10 + 4.500579e10j,
                -1.991163e10 + 6.846221e10j,
            ],
            rtol=1e-3,
        )
    ...
    if plot:
        sim.plot_3d(
            solution=mode_j["J", :, "modes", 0],
            output_format="mayavi",
            compress_scalars=3,
        )
        sim.plot_3d(
            solution=mode_j["J", :, "modes", 1],
            output_format="mayavi",
            compress_scalars=3,
        )
        sim.plot_3d(
            solution=mode_j["J", :, "modes", 2],
            output_format="mayavi",
            compress_scalars=3,
        )


def test_surface_normals(plot=False, skip_asserts=False, write_reference=False):
...
def horseshoe_extinction_modes():
    sim = openmodes.Simulation(
        name="horseshoe_extinction_modes", basis_class=openmodes.basis.LoopStarBasis
    )
    ...
    for freq_count, s in sim.iter_freqs(freqs):
        ...
        extinction_eem[freq_count] = [
            np.vdot(V, j_eem[:, mode])
            * np.dot(V, j_eem[:, mode])
            / z_eem[freq_count, mode]
