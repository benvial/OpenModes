...
def generate_mesh():
    ...
    meshed_name = gmsh.mesh_geometry(
        osp.join(openmodes.geometry_dir, "sphere.geo"),
        input_dir,
        parameters={"radius": 1, "mesh_tol": 0.3},
    )
...
def pec_sphere_multipoles(plot=False):
    ...
    if plot:
        ...
        plt.plot(
            ...
            * sum(
                sum(
                    (np.abs(a_e[l, m]) ** 2 + np.abs(a_m[l, m]) ** 2)
                    for m in range(-l, l + 1)
                )
        ...
        for l in range(1, multipole_order + 1):
            for m in (-1, 1):
                ...
                plt.plot(
                    k0r, -np.pi / k0r**2 * np.sqrt(2 * l + 1) * a_m[l, m].real, "--"
                )
        ...
        return {
            ...
            "results": {"k0r": k0r, "extinction": extinction, "a_e": a_e, "a_m": a_m},
