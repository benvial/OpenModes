...
def geometry_extinction_modes(
    ...
    sim = openmodes.Simulation(name=name, basis_class=openmodes.basis.LoopStarBasis)
    mesh = sim.load_mesh(
        osp.join(openmodes.geometry_dir, name + ".geo"),
        mesh_tol=mesh_tol,
    ...
    for freq_count, s in sim.iter_freqs(freqs):
        ...
        extinction_eem[freq_count] = [
            np.vdot(V, j_eem[:, mode]) * V.dot(j_eem[:, mode]) / z_eem[freq_count, mode]
            for mode in range(num_modes)
        ]
...
geometry_extinction_modes(
    "horseshoe_rect", np.linspace(1e8, 20e9, 101), 3, 1.5e-3, plot_currents=True
)
