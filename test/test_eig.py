...
from openmodes.eig import eig_newton, eig_newton_bordered
...
def test_newton(print_output=False):
    ...
    print(
        "Normalisation of eigenvector:", np.dot(result["eigvec"], result["eigvec_left"])
    )
    print()
    ...
    result = eig_newton(
        func, w_0, vr_0, y_0=vl_0, lambda_tol=1e-10, weight="rayleigh asymmetric"
    )
    assert_allclose(w[0], result["eigval"])
    amax = np.argmax(vr[:, 0])
    ratio = vr[amax, 0] / result["eigvec"][amax]
    ...
    result = eig_newton(
        func, w_0, vr_0, y_0=vl_0, weight="rayleigh asymmetric", lambda_tol=1e-10
    )
    assert_allclose(w[0], result["eigval"])
    amax = np.argmax(vr[:, 0])
    ratio = vr[amax, 0] / result["eigvec"][amax]
